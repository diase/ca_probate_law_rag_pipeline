...
def main():
    url = 'file:///C:/Users/ericl/OneDrive/Documents/probate_sites/self_help_formal_probate.htm'
    soup = scrape_local_file(url)

    bad_div = soup.find("div", class_="jcc_hero__content-top") 
    if bad_div: 
        bad_div.decompose()
        print("removed unwwanted div")

    bad_div = soup.find("div", class_="jcc-share__container") 
    if bad_div: 
        bad_div.decompose()
        print("removed unwwanted div")
    ...
    content_area = soup.find("div", class_="page__content")
    ...
    for tag in content_area.find_all(["h1", "h2", "h3", "h4", "h5", "h6", "p", "li", "a"]):
    ...
    with open("docs/self_help_formal_probate.txt", "w", encoding="utf-8") as f:
